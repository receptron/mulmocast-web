...
const lang = {
  ...
  docs: {
    ...
    slides: {
      title: "MulmoCast Slides",
      description: "Keynote/PPTX/PDF/Marp/MarkdownをMulmoScriptに変換するCLIツール。",
      item1: "チュートリアル: PDF → 動画",
      item2: "CLIコマンド一覧",
      item3: "Markdownコンバーター",
      pageSubtitle:
        "プレゼンテーションファイル（Keynote, PowerPoint, PDF, Marp, Markdown）をMulmoScript形式に変換。LLMによるナレーション自動生成や動画・バンドル作成までワンコマンドで実行可能。",
      toc: {
        title: "目次",
        overview: "概要",
        install: "インストール",
        quickstart: "クイックスタート",
        cli: "CLIコマンド",
        markdown: "Markdownコンバーター",
        tutorial: "チュートリアル: PDF → 動画",
      },
      overview: {
        title: "概要",
        description:
          "MulmoCast Slidesは、プレゼンテーションファイル（Keynote, PowerPoint, PDF, Marp, Markdown）をMulmoScript形式に変換するCLIツールです。スライド画像の抽出、LLMによるナレーション自動生成、動画・バンドル作成までワンコマンドで実行できます。",
        formatHeader: "対応フォーマット",
        extensionHeader: "拡張子",
        requirementsHeader: "必要条件",
      ...
      install: {
        ...
        requirementHeader: "ツール",
        purposeHeader: "用途",
        requiredHeader: "必須",
        nodePurpose: "ランタイム",
        imageMagickPurpose: "画像変換",
        ghostscriptPurpose: "PDF/PPTX処理",
        libreOfficePurpose: "PPTX→PDF変換",
        keynotePurpose: "Keynote→PDF変換",
        required: "必須",
        pdfPptxOnly: "PDF/PPTXのみ",
        pptxOnly: "PPTXのみ",
        keynoteOnly: "macOS + Keynoteのみ",
      },
      quickstart: {
        title: "クイックスタート",
        description: "各フォーマットの基本的な変換コマンドです。",
        narration: "ナレーション自動生成",
        narrationDesc:
          "-g オプションを追加すると、LLM（OpenAI GPT-4o）がスライド内容からナレーションを自動生成します。",
        narrationNote: "注意：",
        narrationNoteText: "ナレーション生成にはOPENAI_API_KEYの設定が必要です。",
      },
      cli: {
        title: "CLIコマンド",
        description: "mulmo-slide コマンドで利用可能なサブコマンド一覧です。",
        commandHeader: "コマンド",
        descriptionHeader: "説明",
        convertDesc: "ファイル形式を自動判定して変換",
        keynoteDesc: "Keynote (.key) をMulmoScriptに変換",
        pptxDesc: "PowerPoint (.pptx) をMulmoScriptに変換",
        pdfDesc: "PDF (.pdf) をMulmoScriptに変換",
      ...
      markdown: {
        title: "Markdownコンバーター",
        description:
          "Markdownファイルをスライド（beat）に分割する際のセパレーターモードを選択できます。カスタム正規表現パターンも指定可能です。",
        modesTitle: "セパレーターモード",
        modeHeader: "モード",
        separatorHeader: "区切り",
        modeDescHeader: "説明",
        hrDesc: "水平線（---）で分割",
        headingDesc: "すべての見出し（#, ##, ###）で分割",
        heading1Desc: "h1（#）で分割",
        heading2Desc: "h2（##）で分割",
        heading3Desc: "h3（###）で分割",
        blankDesc: "連続する空行で分割",
        commentDesc: "HTMLコメントで分割",
        pageBreakDesc: "改ページで分割",
        pluginsTitle: "プラグイン",
        pluginsDescription: "Markdownの変換時に追加のプラグインフラグを指定できます。",
      },
      tutorial: {
        title: "チュートリアル: PDF → ナレーション動画",
        description:
          "PDFやプレゼンテーションファイルから、AIナレーション付き動画を作成する実践的なワークフローです。Claude Code の /narrate スキルを使います。",
        step1Title: "Step 1: プロジェクトディレクトリを作成",
        step1Desc: "作業用のディレクトリを作成し、移動します。",
        step2Title: "Step 2: APIキーを設定",
        step2Desc: "プロジェクトルートに .env ファイルを作成し、OpenAI APIキーを設定します。",
        step3Title: "Step 3: インストール＆スキル初期化",
        step3Desc: "MulmoCast Slides をインストールし、Claude Code スキル（/narrate）を使えるように初期化します。",
        step4Title: "Step 4: /narrate でナレーション生成",
        step4Desc:
          "プロジェクトディレクトリにPDFファイルを配置し、Claude Code で /narrate を実行します。各ページの画像抽出、テキスト抽出、AIによるナレーション生成、メタデータ付与が自動で行われます。",
        outputTitle: "生成されるファイル",
        step5Title: "Step 5: 内容を活用する",
        step5Desc:
          "生成された ExtendedScript を使って、内容への質問応答や要約の生成ができます。mulmocast-preprocessor を使用します。",
        step6Title: "Step 6: ナレーション動画を生成",
        step6Desc: "ExtendedScript を MulmoScript に変換し、MulmoCast CLI で動画を生成します。",
        outputNote:
          "動画は output/ ディレクトリに生成されます。ナレーションの品質を確認し、必要に応じて extended_script.json の text フィールドを編集して再生成できます。",
        formatsTitle: "対応フォーマット",
        troubleshootingTitle: "トラブルシューティング",
        symptomHeader: "症状",
        solutionHeader: "解決策",
        scriptMetaFix:
          "MulmoScript JSON から scriptMeta, outputProfiles, beats[].meta, beats[].variants を手動で削除してください。",
        emptyNarration: "ナレーションが空",
        emptyNarrationFix:
          "/narrate を実行済みか確認してください。PDF変換はテキストを意図的に空にするため、/narrate によるナレーション生成が必要です。",
        imageMagickFix:
          "ImageMagick をインストールしてください。macOS: brew install imagemagick / Ubuntu: sudo apt-get install imagemagick",
      },
      links: "リンク",
    },
    easy: {
